...
@RestController
// 공통 URL 경로 설정
@RequestMapping("v7/posts")
public class PostApi7 {
...
//        if(title.trim().isBlank()) 와 동일
        // 제목이 아예 없거나 비어있는 경우
        if(title == null || title.isBlank()) {
            throw new RuntimeException("title을 입력해주세요.");
        }

//        if(content.trim().isBlank()) 와 동일
...
//        if(newTitle.trim().isBlank()) 와 동일
        if(newTitle == null || newTitle.isBlank()) {
            throw new RuntimeException("수정할 title을 입력해주세요.");
        }

//        if(newContent.trim().isBlank()) 와 동일
        if(newContent == null || newContent.isBlank()) {
            throw new IllegalArgumentException("수정할 Content를 입력해주세요.");
        }
